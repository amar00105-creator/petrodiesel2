...
import { X, Vault, ArrowUpRight, ArrowDownRight, ArrowRightLeft, CreditCard, Calendar, Trash2, Edit } from 'lucide-react';
...
export default function SafeDetailsModal({ isOpen, onClose, safeId, currency = 'SDG' }) {
    ...
    const fetchDetails = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`/PETRODIESEL2/public/finance/getSafeDetails?id=${safeId}`, {
            ...
            if (result.success) {
                ...
                setEditForm({ name: result.safe.name });
            } else {
                toast.error(result.message || 'فشل تحميل بيانات الخزينة');
                onClose();
            }
        } catch (error) {
            console.error(error);
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('/PETRODIESEL2/public/finance/updateSafe', {
                method: 'POST',
                body: formData,
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            });
            ...
            if (result.success) {
                toast.success('تم تحديث بيانات الخزينة بنجاح');
                setIsEditing(false);
    ...
    const handleDelete = async () => {
        try {
             ...
             const response = await fetch('/PETRODIESEL2/public/finance/deleteSafe', {
                 method: 'POST',
                 body: formData,
                 headers: { 'X-Requested-With': 'XMLHttpRequest' }
             });
            ...
            {isOpen && (
                <>
                    ...
                    <DeleteConfirmationModal 
                        ...
                        title="حذف الخزينة"
                        message="هل أنت متأكد من حذف هذة الخزينة؟ سيتم حذف جميع السجلات المرتبطة بها."
                    />
                    ...
                    >
                        <div className="bg-white pointer-events-auto rounded-[2rem] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden ring-1 ring-slate-900/5">
                            ...
                            {/* Header / Banner - Blue Theme for Safes */}
                            <div className="relative bg-gradient-to-br from-blue-600 to-blue-800 p-8 text-white overflow-hidden shrink-0">
                                {/* Decorative Circles */}
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div>
                                                {loading ? (
                                                    ...
                                                    <h2 className="text-3xl font-bold">{data.safe?.name}</h2>
                                                )}
                                                <div className="flex items-center gap-2 text-blue-100 opacity-90">
                                                    <span className="font-medium text-sm">خزينة نقدية</span>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="mt-8">
                                    <div className="text-blue-200 text-sm mb-1 uppercase tracking-wider font-semibold">الرصيد الحالي</div>
                                    {loading ? (
                                        ...
                                        {loading ? (
                                            ...
                                            <div className="p-12 text-center text-slate-400">
                                                <div className="bg-slate-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3">
                                                    <Vault className="w-8 h-8 text-slate-300" />
                                                </div>
                                                <p>لا توجد عمليات مسجلة لهذة الخزينة</p>
                                            </div>
                                        ) : (
                                            data.transactions.map((t, idx) => (
                                                ...
                                                >
                                                    ...
                                                    <div className={`text-xl font-bold font-mono ${
                                                        t.type === 'income' || (t.type === 'transfer' && t.to_id == safeId) ? 'text-emerald-600' :
                                                        'text-rose-600'
                                                    }`}>
                                                        {t.type === 'expense' || (t.type === 'transfer' && t.from_id == safeId) ? '-' : '+'}
