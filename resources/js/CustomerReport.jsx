        ...
        <div className="space-y-6 animate-fade-in pb-10">
            ...
            {!filters.customer_id ? (
                ...
                >
                    ...
                    <Title className="text-slate-700 text-xl font-bold">حدد العميل لعرض التقرير</Title>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
                            <Card decoration="top" decorationColor="emerald" className="bg-white hover:shadow-lg transition-shadow">
                                <Text>إجمالي المبيعات (لهم)</Text>
                                <Metric className="mt-2 text-emerald-600 drop-shadow-sm">{formatCurrency(data.totals.total_sales)}</Metric>
                            </Card>
                        </motion.div>
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
                            <Card decoration="top" decorationColor="blue" className="bg-white hover:shadow-lg transition-shadow">
                                ...
                                <Metric className="mt-2 text-blue-600 drop-shadow-sm">{formatCurrency(data.totals.total_paid)}</Metric>
                            </Card>
                        </motion.div>
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
                            <Card decoration="top" decorationColor={data.totals.net_balance > 0 ? 'teal' : 'rose'} className="bg-white hover:shadow-lg transition-shadow">
                                <Text>صافي الرصيد النهائي</Text>
                                <Metric className={`mt-2 ${data.totals.net_balance > 0 ? 'text-teal-600' : 'text-rose-600'} drop-shadow-sm`}>
                                    ...
                                    <AnimatePresence mode="wait">
                                        {data.transactions.length > 0 ? (
                                            data.transactions.map((row, idx) => (
                                                ...
                                                >
                                                    <td className="p-4 whitespace-nowrap text-slate-500 font-mono text-xs">{row.date}</td>
                                                    <td className="p-4">
                                                        <div className="font-bold text-slate-800">{row.statement_title}</div>
                                                        <div className="text-[10px] text-slate-400 group-hover:text-slate-500">{row.statement_subtitle}</div>
                                                    </td>
                                                    <td className="p-4">
                                                    ...
                                                    <td className="p-4 font-bold" dir="ltr">
                                                        <span className={row.running_balance >= 0 ? 'text-teal-600' : 'text-rose-600'}>
